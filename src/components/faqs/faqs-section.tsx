...
export function FAQsSection({ initialFaqs }: FAQsSectionProps) {
  ...
  const [searchQuery, setSearchQuery] = useState<string>(''); // Explicitly typed as string
  ...
  // Filter FAQs based on search - SIMPLIFIED FILTERING (Only Question)
  useEffect(() => {
    if (!searchQuery.trim()) {
    ...
    // Defensive check for searchQuery being a string before toLowerCase()
    const query = (typeof searchQuery === 'string' ? searchQuery : '').toLowerCase();

    const filtered = faqs.filter(faq => {
      // ONLY check the question for now - drastically simplified
      const questionMatch = faq.question.toLowerCase().includes(query);
      return questionMatch; // Only filter by question, ignoring answer for now
    });
